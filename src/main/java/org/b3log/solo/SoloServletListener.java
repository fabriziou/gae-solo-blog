...
public final class SoloServletListener extends AbstractServletListener {
    ...
    private static  void loadLatke(final ServletRequestEvent servletRequestEvent)  {

        ServletContext servletContext = servletRequestEvent.getServletContext();

        Object o = servletContext.getAttribute("loadLatke");
        ...
        if(o == null || StringUtils.isEmpty(String.valueOf(o))){
            ...
            if(httpServletRequest.isSecure() && port == -1){
                port=443;
            } else if(port == -1) {
                port=80;
            }

            Latkes.setServerHost(domain);
            Latkes.setServerScheme(protocol);
            Latkes.setServerPort(String.valueOf(port));

            Latkes.setStaticServerHost(domain);
            Latkes.setStaticServerScheme(protocol);
            Latkes.setStaticServerPort(String.valueOf(port));

            servletContext.setAttribute("loadLatke","loadLatke");
